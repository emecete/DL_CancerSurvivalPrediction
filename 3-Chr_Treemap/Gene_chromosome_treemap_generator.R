...
start.time <- Sys.time()
print("loading file")
exp_to_tree_map <- read.csv("lost0_exp_to_tree_map.csv") # exp_to_chromosome_tree_map.csv
print("file loaded")
end.time <- Sys.time()
...
maxi <- 19.211 #max(exp_to_tree_map[,samples_col])
...
exp_generate_treemap <- function(i){
    ...
    png(filename=trimws(paste("../data/CHR_TREEMAPS/", sample, "_TREEMAP.png")))
