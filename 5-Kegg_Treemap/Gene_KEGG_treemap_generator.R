...
#DELETE NOT COMMON GENES (IN KEGG)
chr_ens_Ids <- as.character(read.csv("../3-Chr_Treemap/exp_to_chromosome_tree_map.csv")[,'ensId'])
common <- intersect(chr_ens_Ids, exp_to_tree_map[,'ensId'])

exp_to_tree_map <- exp_to_tree_map[which(exp_to_tree_map[,'ensId'] %in% common),]
...
exp_generate_treemap <- function(i){
	...
	start.time <- Sys.time()
	png(filename=paste("../data/KEGG/KEGG_TREEMAPS/", trimws(sample), "_TREEMAP.png"))
