...
start.time <- Sys.time()
print("loading file") 
exp_to_tree_map <- read.csv("lost_to_tree_map.csv") #COMMON_GENES_exp_to_KEGG_tree_map.csv
print("file loaded")
end.time <- Sys.time()
...
exp_generate_treemap <- function(i){
	...
	png(filename=trimws(paste("../data/KEGG/COMMON_GENES_KEGG_TREEMAPS/", trimws(sample), "_TREEMAP.png")))
