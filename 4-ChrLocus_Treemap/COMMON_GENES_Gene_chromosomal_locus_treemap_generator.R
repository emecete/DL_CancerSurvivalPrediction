...
start.time <- Sys.time()
print("loading file")
exp_to_tree_map <- read.csv("lost.csv") #COMMON_GENES_exp_to_chromosome_tree_map.csv
print("file loaded")
end.time <- Sys.time()
...
exp_to_tree_map$gene_start <- as.double(exp_to_tree_map$gene_start)

exp_generate_treemap <- function(i){
	require(treemap)
	colnams <- colnames(exp_to_tree_map)
	sample <- colnams[samples_col[i]]
    png(filename=trimws(paste("../data/COMMON_GENES_CHR_LOCUS_TREEMAPS/", sample, "_TREEMAP.png")))
    par(bty = 'n') # reduce white background around plot in png image
    treemap(exp_to_tree_map,
        index=c("chromosome","ensId"),
        vSize="tamPixel",vColor=sample,sortID = "gene_start",fontsize.labels = 0,border.lwds = 0,
